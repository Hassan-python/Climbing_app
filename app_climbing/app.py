...
def get_advice_from_frames(frames, openai_api_key, problem_type, crux):
    ...
    # 3. ChromaDB Vector Storeの初期化/接続 (Cloud Runへの接続に変更)
    chromadb_url = get_chromadb_url()
    if not chromadb_url:
        # エラーメッセージは get_chromadb_url 内で表示される
        return None, []
    ...
    try:
        ...
        # ポートが指定されていれば取得、なければデフォルト (httpsなら443)
        ...
        st.info(f"ChromaDB ({chromadb_url}) に接続しました。")
    except Exception as e:
        st.error(f"ChromaDBへの接続/初期化に失敗しました: {e}")
        st.error(f"URL: {chromadb_url}, Host: {host}, Port: {port}, SSL: {ssl_enabled}")
